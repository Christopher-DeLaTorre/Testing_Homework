...
class MainTests(unittest.TestCase):
    ...
    def test_book_detail_logged_in(self):
        ...
        # TODO: Make a GET request to the URL /book/1, check to see that the
        # status code is 200
        hold = self.app.get('/book/1', follow_redirects=True)
        self.assertEqual(hold.status_code, 200)
        # TODO: Check that the response contains the book's title, publish date,
        # and author's name
        hold_txt = hold.get_data(as_text=True)
        self.assertIn('To Kill a Mockingbird', hold_txt)
        self.assertIn('1960', hold_txt)
        self.assertIn('Harper Lee', hold_txt)
        # TODO: Check that the response contains the 'Favorite' button
        pass
        self.assertIn('Favorite This Book', hold_txt)
    ...
    def test_create_genre(self):
        ...
        pass
        created_genre = Genre.query.filter_by(name='Comedy').one()
        self.assertIsNotNone(created_genre)
        self.assertEqual(created_genre.name, 'Comedy')

    def test_profile_page(self):
        # TODO: Make a GET request to the /profile/me1 route
        create_user()
        create_books()
        hold = self.app.get('/profile/me1', follow_redirects=True)
        self.assertEqual(hold.status_code, 200)
        # TODO: Verify that the response shows the appropriate user info
        pass
        hold_txt = hold.get_data(as_text=True)
        self.assertIn('me1', hold_txt)
